...
def open_book_crud(root, open_main_window):
    ...
    tk.Label(book_window, text="Titulo:").pack(pady=5)
    title_entry = tk.Entry(book_window, width=30)
    title_entry.pack(pady=5)

    tk.Label(book_window, text="Autor:").pack(pady=5)
    author_entry = tk.Entry(book_window, width=30)
    author_entry.pack(pady=5)

    tk.Label(book_window, text="Precio:").pack(pady=5)
    price_entry = tk.Entry(book_window, width=30)
    price_entry.pack(pady=5)

    tk.Label(book_window, text="Cantidad:").pack(pady=5)
    quantity_entry = tk.Entry(book_window, width=30)
    quantity_entry.pack(pady=5)

    tk.Label(book_window, text="ID de categoria:").pack(pady=5)
    category_entry = tk.Entry(book_window, width=30)
    category_entry.pack(pady=5)

    tk.Button(book_window, text="Agregar libro", command=lambda: add_book(title_entry, author_entry, price_entry, quantity_entry, category_entry, book_list)).pack(pady=5)

    book_list = tk.Listbox(book_window, width=60, height=10)
    book_list.pack(pady=10)

    tk.Button(book_window, text="Actualizar libro seleccionado", command=lambda: update_book(book_list)).pack(pady=5)
    tk.Button(book_window, text="Eliminar libro seleccionado", command=lambda: delete_book(book_list)).pack(pady=5)

    update_book_list(book_list)

    tk.Button(book_window, text="Volver", command=lambda: go_back(book_window, open_main_window)).pack(pady=10)
